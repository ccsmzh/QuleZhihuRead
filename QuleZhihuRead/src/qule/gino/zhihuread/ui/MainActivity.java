...
import qule.gino.zhihuread.ui.widget.RefreshListView;
...
public class MainActivity extends BaseActivity implements LoaderCallbacks<List<HashMap<String, Object>>> {
    ...
    private RefreshListView mContentListView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContentListView = (RefreshListView) findViewById(R.id.m_lv_contesnt);
...
//                    mContentListView.preparePullTrack();
//                }
        ...
        switch (load.getId()) {
            ...
            case LOADER_MORE_ID:
                if (data == null || data.size() == 0) {
                    ...
                    switch (mContentListView.getLastDirection()) {
                        case RefreshListView.LEFT_DIRECTION:
                            Log.d(TAG,"左边执行");
                            LayoutInflater inflate = (LayoutInflater)
                                    MainActivity.this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                            View v = inflate.inflate(R.layout.refresh_listview_msg_item, null);
                            ((TextView)v.findViewById(R.id.rlmi_tv_content)).setText("左边执行");
                            ViewGroup.LayoutParams layoutParams = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                            addContentView(v, layoutParams);
                            break;
                        case RefreshListView.RIGHT_DIRECTION:
                            Log.d(TAG,"右边执行");
                            inflate = (LayoutInflater)
                                    MainActivity.this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                            v = inflate.inflate(R.layout.refresh_listview_msg_item, null);
                            ((TextView)v.findViewById(R.id.rlmi_tv_content)).setText("右边执行");
